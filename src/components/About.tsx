import Image from 'next/image'
import Link from 'next/link';

const Card = () => {
  return (
    <div className="shadow-xl w-[200px] z-10 bg-white px-4 py-8 rounded-xl">
      <p className="py-1 px-3 bg-purple-100 text-purple-800 rounded w-fit text-sm">Featured</p>
      <p className="text-xl font-medium my-2">The map of mathematics</p>
      <p className="text-gray-600 text-sm mb-6">Egestas elit dui scelerisque ut eu purus aliquam vitae habitasse.</p>
      <Link href="/sign-up" className="text-sm mr-2 px-8 py-3 border-2 text-blue-600 font-semibold rounded-lg hover:bg-blue-600 hover:text-white">
        Take Lesson
      </Link>
    </div>
  )
}

export default function About() {
  return (
    <div className="container mx-auto px-4 py-4 flex h-[700px] items-center">
      <div className="py-4 flex flex-col w-1/2">
        <h2 className="text-5xl text-gray-900 font-extrabold relative">
          <span>An all-in-one app that makes it easier</span>
          <img src="underlineabout.svg" alt="conteúdo" class="block top-10 left-15 absolute" />
        </h2>
        <ul className="mt-12">
          <li className="flex">
            <span className="mr-1"><Image src="/check.svg" width={24} height={24} /></span>
            Est et in pharetra magna adipiscing ornare aliquam.
          </li>
          <li className="flex">
            <span className="mr-1"><Image src="/check.svg" width={24} height={24} /></span>
            Tellus arcu sed consequat ac velit ut eu blandit.
          </li>
          <li className="flex">
            <span className="mr-1"><Image src="/check.svg" width={24} height={24} /></span>
            Ullamcorper ornare in et egestas dolor orci.
          </li>
        </ul>
        <Link href="/login" className="mt-8 py-2 items-center border-0 border-black text-blue-600 font-semibold rounded-lg hover:border-2 hover:border-blue-600 flex">
          Find more about the app <span className="ml-1"><Image src="/arrow_right.svg" width={12} height={12} /></span>
        </Link>
      </div>
      <div className="w-1/2 px-4 py-4 flex flex-col relative h-full items-center">
        <img src="blobabout.svg" alt="conteúdo" className= "
          z-1
          absolute
          left-0
          top-30
          h-[400px]
          transform
          bg-no-repeat
        "/>
        <img src="desktopabout.svg" alt="conteúdo" class="z-2 absolute top-10" />
        <img src="abouticons.svg" alt="conteúdo" class="z-3 block top-0 left-0 w-42 h-full absolute" />
        <div className="z-10 absolute bottom-10 left-5 flex justify-between gap-4">
          {[1, 2, 3].map(item => <Card />)}
        </div>
      </div>
    </div>
  );
}
